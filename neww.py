...
SHOP_URL = "https://kays-media.myshopify.com"
VARIANT_ID = None
...
def step4_submit_completion(session, checkout_token, session_token, queue_token, 
    ...
    _last_error_result = None
    for _retry_attempt in range(3):
        if _retry_attempt > 0:
            print(f"  [RETRY] ConfirmChangeViolation retry {_retry_attempt + 1}/3 with fresh attempt token...")
            attempt_token = f"{checkout_token}-{uuid.uuid4().hex[:10]}"
            payload["variables"]["attemptToken"] = attempt_token
            time.sleep(1)
        ...
        if r.status_code == 200:
            ...
            if result_type in ['SubmitSuccess', 'SubmitAlreadyAccepted', 'SubmittedForCompletion']:
                ...
                if any('ConfirmChangeViolation' in c for c in error_codes):
                    print(f"  [RETRY] ConfirmChangeViolation detected, will retry...")
                    _last_error_result = (None, primary_code, combined_message, response)
                    continue
    ...
    if _last_error_result:
        print(f"  [ERROR] All ConfirmChangeViolation retry attempts exhausted")
        return _last_error_result
    return None, "RETRY_EXHAUSTED", "All retry attempts failed", {}
...
def step4_submit_completion_ctx(session, checkout_token, session_token, queue_token,
    ...
    for _retry_attempt in range(3):
        ...
        if r.status_code == 200:
            ...
            if result_type in ['SubmitSuccess', 'SubmitAlreadyAccepted', 'SubmittedForCompletion']:
                ...
                if any('ConfirmChangeViolation' in c for c in error_codes):
                    print(f"  [RETRY] ConfirmChangeViolation detected, will retry...")
                    _last_error_result = (None, primary_code, combined_message, response, actual_total)
                    continue
    ...
    if _last_error_result:
        print(f"  [ERROR] All ConfirmChangeViolation retry attempts exhausted")
        return _last_error_result
    return (None, "RETRY_EXHAUSTED", "All retry attempts failed", {}, actual_total)
